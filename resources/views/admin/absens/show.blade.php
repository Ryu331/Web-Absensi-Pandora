...
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.absens.index') }}" class="text-blue-600 hover:underline text-sm">Kembali ke Data Absensi</a>
    <h2 class="text-2xl font-bold mt-2">Detail Absensi</h2>
</div>
    ...
    <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow p-5">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
                ...
                <dd class="font-medium">{{ $absen->user?->name ?? 'N/A' }}</dd>
            </div>
            <div>
                <dt class="text-gray-500">Email</dt>
                <dd class="font-medium break-all">{{ $absen->user?->email ?? '-' }}</dd>
            </div>
            <div>
...
<div class="mt-6 bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
    <div class="p-5 border-b dark:border-gray-700">
        <h3 class="font-semibold">Riwayat Validasi</h3>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-4 py-8 text-center text-gray-400">Belum ada request validasi</td>
                </tr>
                @endforelse
            </tbody>
        </table>
